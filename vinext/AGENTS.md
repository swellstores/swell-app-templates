...
- Backend credentials stay in Worker code. Validate the existing `_swell_admin_session` against Swell and authorize each protected operation. Trusted store headers are not user authentication. Never create an unrestricted Backend API proxy.
...
- Keep this template independently scaffoldable. No imports from sibling templates or the repository root.
